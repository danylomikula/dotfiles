...
## Git conventions
- Branch names must be entirely lowercase.
  - Use `-` as a separator if needed, for example:
    - `feature/add-helm-module`
    - `bugfix/fix-vault-role`
  - Do not use spaces, uppercase letters, or special characters outside `-` and `/`.
- Commit messages must also be entirely lowercase.
  - Prefer short, imperative-style messages in lowercase, for example:
    - `add talos cluster config`
    - `update pyenv version`
    - `fix vault oidc role binding`
